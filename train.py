...
# Metrics
tr_psnr = tf.image.psnr(tf.to_int32(tf.clip_by_value(tr_infer, 0., 1.) * (2**high - 1)), tf.to_int32(tr_label * (2**high - 1)), max_val=(2**high - 1))
tr_ssim = tf.image.ssim(tf.to_int32(tf.clip_by_value(tr_infer, 0., 1.) * (2**high - 1)), tf.to_int32(tr_label * (2**high - 1)), max_val=(2**high - 1))
tr_mse = tf.reduce_mean(tf.square(tf.clip_by_value(tr_infer, 0., 1.) - tr_label))
